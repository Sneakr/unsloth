...
function detectTrainingCapabilities(opts: {
  ...
  if (opts.pipelineTag) tagSet.add(opts.pipelineTag.toLowerCase());
  return {
...
export function resolvePickerInferredModelType(
  ...
  if (current === "vision" && flags.isVision) return current;
  if (current === "audio" && flags.isAudio) return current;
  if (current === "embeddings" && flags.isEmbedding) return current;
