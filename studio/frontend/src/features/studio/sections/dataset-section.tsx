...
export function DatasetPanel() {
  ...
  useEffect(() => {
    ...
    // Preserve explicitly-provided datasets: an entered local path (a
    // save_to_disk directory, drive/UNC path, etc.) or a direct file upload.
    // These legitimately never appear in the recipe-dataset scan, so absence
    // from it must not silently wipe them — the backend dataset check surfaces
    // a real missing-path error instead.
  ...
  return (
    <div className="flex min-w-0 flex-col gap-4">
      ...
      {datasetSource !== "s3" && (tokenValidationError ?? null) && (
      ...
      {datasetSource !== "s3" && isCheckingToken && (
